 ...
 *
 * weighted.c
 *	  support routines for weighted tablesample method
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/tablesample/weighted.c
 *
 *-------------------------------------------------------------------------
 */
...
static void weighted_samplescangetsamplesize(PlannerInfo *root,
                                              RelOptInfo *baserel,
                                              List *paramexprs,
                                              BlockNumber *pages,
                                              double *tuples);
static void weighted_initsamplescan(SampleScanState *node,
                                     int eflags);
static void weighted_beginsamplescan(SampleScanState *node,
                                      Datum *params,
                                      int nparams,
                                      uint32 seed);
static OffsetNumber weighted_nextsampletuple(SampleScanState *node,
                                              BlockNumber blockno,
                                              OffsetNumber maxoffset);

/*
 * Create a TsmRoutine descriptor for the weighted method.
 */
Datum
    tsm_weighted_handler(PG_FUNCTION_ARGS)
{
    ...
    tsm->SampleScanGetSampleSize = weighted_samplescangetsamplesize;
    tsm->InitSampleScan = weighted_initsamplescan;
    tsm->BeginSampleScan = weighted_beginsamplescan;
    tsm->NextSampleBlock = NULL;
    tsm->NextSampleTuple = weighted_nextsampletuple;
 ...
 */
static void
weighted_samplescangetsamplesize(PlannerInfo *root,
                                  RelOptInfo *baserel,
                                  List *paramexprs,
                                  BlockNumber *pages,
                                  double *tuples)
 ...
 */
static void
weighted_initsamplescan(SampleScanState *node, int eflags)
{
    node->tsm_state = palloc0(sizeof(WeightedSamplerData));
}
 ...
 */
static void
weighted_beginsamplescan(SampleScanState *node,
                          Datum *params,
                          int nparams,
                          uint32 seed)
{
    WeightedSamplerData *sampler = (WeightedSamplerData *)node->tsm_state;
 ...
 */
static OffsetNumber
weighted_nextsampletuple(SampleScanState *node,
                          BlockNumber blockno,
                          OffsetNumber maxoffset)
{
    WeightedSamplerData *sampler = (WeightedSamplerData *)node->tsm_state;
