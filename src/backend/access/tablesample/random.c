 ...
 *
 * random.c
 *	  support routines for random tablesample method
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/tablesample/random.c
 *
 *-------------------------------------------------------------------------
 */
...
static void random_samplescangetsamplesize(PlannerInfo *root,
                                           RelOptInfo *baserel,
                                           List *paramexprs,
                                           BlockNumber *pages,
                                           double *tuples);
static void random_initsamplescan(SampleScanState *node,
                                  int eflags);
static void random_beginsamplescan(SampleScanState *node,
                                   Datum *params,
                                   int nparams,
                                   uint32 seed);
static OffsetNumber random_nextsampletuple(SampleScanState *node,
                                           BlockNumber blockno,
                                           OffsetNumber maxoffset);

/*
 * Create a TsmRoutine descriptor for the random method.
 */
Datum
    tsm_random_handler(PG_FUNCTION_ARGS)
{
    ...
    tsm->SampleScanGetSampleSize = random_samplescangetsamplesize;
    tsm->InitSampleScan = random_initsamplescan;
    tsm->BeginSampleScan = random_beginsamplescan;
    tsm->NextSampleBlock = NULL;
    tsm->NextSampleTuple = random_nextsampletuple;
 ...
 */
static void
random_samplescangetsamplesize(PlannerInfo *root,
                               RelOptInfo *baserel,
                               List *paramexprs,
                               BlockNumber *pages,
                               double *tuples)
 ...
 */
static void
random_initsamplescan(SampleScanState *node, int eflags)
{
    node->tsm_state = palloc0(sizeof(RandomSamplerData));
}
 ...
 */
static void
random_beginsamplescan(SampleScanState *node,
                       Datum *params,
                       int nparams,
                       uint32 seed)
{
    RandomSamplerData *sampler = (RandomSamplerData *)node->tsm_state;
 ...
 */
static OffsetNumber
random_nextsampletuple(SampleScanState *node,
                       BlockNumber blockno,
                       OffsetNumber maxoffset)
{
    RandomSamplerData *sampler = (RandomSamplerData *)node->tsm_state;
